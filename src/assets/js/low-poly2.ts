...
export function draw(container: Element, refreshDuration: number = 10000) {
  ...
  for (var i = 0; i < points.length; i++) {
    if (points[i].originX !== unitWidth * (numPointsX - 1) && points[i].originY !== unitHeight * (numPointsY - 1)) {
      ...
      for (var n = 0; n < 2; n++) {
        var polygon = document.createElementNS(svg.namespaceURI, 'polygon') as any
        ...
        if (rando === 0) {
          if (n === 0) {
            ...
            polygon.setAttribute(
              'points', topLeftX + ','
              + topLeftY + ' ' + topRightX + ',' + topRightY + ' ' + bottomRightX + ',' + bottomRightY)
          }
        } else if (rando === 1) {
          if (n === 0) {
            polygon.point1 = i
            ...
            polygon.setAttribute(
              'points', topLeftX + ',' +
              topLeftY + ' ' + bottomLeftX + ',' + bottomLeftY + ' ' + topRightX + ',' + topRightY)
          } else if (n === 1) {
            ...
            polygon.setAttribute(
              'points', bottomLeftX + ',' +
              bottomLeftY + ' ' + topRightX + ',' + topRightY + ' ' + bottomRightX + ',' + bottomRightY)
          }
        }
...
function refresh(container: Element, refreshDuration: number = 10000) {
  ...
  refreshTimeout = setTimeout(function () { refresh(container, refreshDuration) }, refreshDuration)
}
